...
template<typename T>
size_t pcgTransSharedMemSize(uint32_t state_size, uint32_t knot_points, int pcg_poly_order) {
    if (pcg_poly_order == 1) {
...
template<typename T>
bool checkPcgTransOccupancy(void *kernel, dim3 block, uint32_t state_size, uint32_t knot_points, int poly_order) {

    const uint32_t smem_size = pcgTransSharedMemSize<T>(state_size, knot_points, poly_order);
    int dev = 0;
...
void pcgTrans(
    ...
    for (unsigned ind = thread_id; ind < state_size; ind += block_dim) {
        ...
        // s_r_b is already the middle part of s_r
    }
    grid.sync();
    ...
    for (unsigned ind = thread_id; ind < state_size; ind += block_dim) {
        ...
        // s_p_b is already the middle part of s_p
    }
