                                                                    ...
                                                                    uint32_t&                    d_iterations)
{
        ...
        if (abs(s_rho) < abs_tol) {
                if (threadIdx.x == 0) { d_iterations = 0; }
                __syncthreads();
                return;
        }
...
#pragma unroll
                ...
                if (abs(s_rho_new) < (abs_tol)) { break; }
...
#pragma unroll
        ...
        if (threadIdx.x == 0) { d_iterations = iterations; }
...
template<typename T, uint32_t BatchSize>
__host__ void solvePCGBatched(T* d_lambda_batch, const T* d_S_batch, const T* d_P_inv_batch, const T* d_gamma_batch, uint32_t max_pcg_iters, uint32_t& d_iterations)
{
        dim3           grid(BatchSize);
        ...
        solvePCGBatchedKernel<T, BatchSize>
            <<<grid, thread_block, s_mem_size>>>(d_lambda_batch, d_S_batch, d_P_inv_batch, d_gamma_batch, max_pcg_iters, d_iterations);
}
