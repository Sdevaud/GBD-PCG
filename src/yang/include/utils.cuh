                          ...
                          uint32_t block_id)
{
    ...
    uint32_t warp_id = tid >> 5;     // /32
    uint32_t lane    = tid % 32;     // lane id

    uint32_t numCols    = (block_id == 0 || block_id == max_block_id) ? 2u * b_dim : 3u * b_dim;
    uint32_t mat_offset = (block_id == 0) ? b_dim * b_dim : 0u;
    uint32_t vec_offset = (block_id == 0) ? b_dim : 0u;

    // each warp compute one row r
    for (uint32_t r = warp_id; r < b_dim; r += (blockDim.x >> 5)) {

        T val = static_cast<T>(0);

        // each lane handles columns c = lane, lane+32, ...
        for (uint32_t c = lane; c < numCols; c += warpSize) {
            val += s_mat[mat_offset + b_dim * c + r] * s_vec[vec_offset + c];
        }

        // warp reduction
        for (uint32_t off = warpSize >> 1; off > 0; off >>= 1) {
